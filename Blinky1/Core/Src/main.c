...
/* Delay table - initialize at declaration to avoid race condition */
...
static void UpdateLED(void)
{
  static uint8_t toggle_count = 0;  // Track complete blink cycles
  ...
  if (led_timer >= current_delays[delay_idx])
  {
    ...
    {
      toggle_count = 0;
      delay_idx += direction;  // Auto-advance to next delay
      WrapIndex();
    }
  }
}
...
static void UpdateButton(void)
{
  ...
  if (btn_stable && !btn_prev)
  {
    ...
    if (btn_clicks < 2)  // cap at 2 (triple+ = double)
      btn_clicks++;
  ...
  if (btn_stable)
  {
    ...
    {
      btn_hold = 1;
      btn_clicks = 0;  // cancel click logic
      btn_click_timer = 0;
  ...
  if (!btn_stable && btn_prev)
  {
    ...
    btn_hold = 0;
    btn_click_timer = 0;  // start click resolution window
  }
  ...
  if (!btn_hold && !btn_stable && (btn_clicks > 0))
  {
    ...
    if (btn_click_timer >= CLICK_WINDOW_MS)
    {
      if (btn_clicks == 1)
      {
        ...
        delay_idx += direction;
        WrapIndex();
        led_timer = 0;  // restart timing from new delay
      }
      else
      {
...
int main(void)
{
  ...
  while (1)
  {
    /* All logic handled in HandleTick() via SysTick callback */
  }
  /* USER CODE END WHILE */
...
/*
 * HAL_SYSTICK_Callback() - called every 1ms by HAL
 * Keep minimal - just call HandleTick()
 */
